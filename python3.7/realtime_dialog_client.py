...
class RealtimeDialogClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # 尝试捕获 InvalidStatusCode 并打印返回头用于排查（401/403 等）
            try:
                from websockets.exceptions import InvalidStatusCode
                if isinstance(e, InvalidStatusCode):
                    status = getattr(e, 'status_code', None)
                    resp_headers = getattr(e, 'response_headers', None)
                    print(f"Failed to connect to upstream WebSocket: status={status}")
                    if resp_headers:
                        print("Upstream response headers:")
